...
const salaryCalculatorFAQs = [
  ...
  },
  {
    ...
    answer:
      'UK income tax is calculated using a progressive system with different rates: 0% (Personal Allowance up to £12,570), 20% (Basic Rate £12,571-£50,270), 40% (Higher Rate £50,271-£125,140), and 45% (Additional Rate above £125,140). Scotland has different rates.',
  },
  {
    question: 'What is National Insurance and how much do I pay?',
    answer:
      'National Insurance funds state benefits like the NHS and state pension. For 2025/26, you pay 8% on earnings between £12,570-£50,270, then 2% on earnings above £50,270.',
  },
  {
  ...
  },
  {
    ...
    answer:
      "Yes! Pension contributions are deducted from your gross salary before tax is calculated, reducing your taxable income. The annual allowance is typically £40,000 (or £10,000 if you're a high earner already drawing pension benefits).",
  },
  {
    question: 'What does my tax code mean?',
    answer:
      "Your tax code tells your employer how much tax-free income you're entitled to. The standard code 1257L gives you the full £12,570 personal allowance. Different codes reflect your personal circumstances.",
  },
  {
  ...
  const handleCalculate = () => {
    ...
    if (newResults.grossAnnual !== undefined) {
      const csvExportData = [
        ...
        [
          'Gross Salary',
          `£${newResults.grossAnnual.toFixed(2)}`,
          `£${(newResults.grossAnnual / 12).toFixed(2)}`,
          `£${(newResults.grossAnnual / 52).toFixed(2)}`,
          `£${(newResults.grossAnnual / 260).toFixed(2)}`,
        ],
        ['Personal Allowance', `£${newResults.personalAllowance.toFixed(2)}`, '', '', ''],
        [
          'Income Tax',
          `£${(-newResults.tax.total).toFixed(2)}`,
          `£${(-newResults.tax.total / 12).toFixed(2)}`,
          `£${(-newResults.tax.total / 52).toFixed(2)}`,
          `£${(-newResults.tax.total / 260).toFixed(2)}`,
        ],
        [
          'National Insurance',
          `£${(-newResults.nationalInsurance.total).toFixed(2)}`,
          `£${(-newResults.nationalInsurance.total / 12).toFixed(2)}`,
          `£${(-newResults.nationalInsurance.total / 52).toFixed(2)}`,
          `£${(-newResults.nationalInsurance.total / 260).toFixed(2)}`,
        ],
        [
          'Pension Contribution',
          `£${(-newResults.pension).toFixed(2)}`,
          `£${(-newResults.pension / 12).toFixed(2)}`,
          `£${(-newResults.pension / 52).toFixed(2)}`,
          `£${(-newResults.pension / 260).toFixed(2)}`,
        ],
        [
          'Student Loan',
          `£${(-newResults.studentLoan).toFixed(2)}`,
          `£${(-newResults.studentLoan / 12).toFixed(2)}`,
          `£${(-newResults.studentLoan / 52).toFixed(2)}`,
          `£${(-newResults.studentLoan / 260).toFixed(2)}`,
        ],
        [
          'Total Deductions',
          `£${(-newResults.totalDeductions).toFixed(2)}`,
          `£${(-newResults.totalDeductions / 12).toFixed(2)}`,
          `£${(-newResults.totalDeductions / 52).toFixed(2)}`,
          `£${(-newResults.totalDeductions / 260).toFixed(2)}`,
        ],
        [
          'Net Take-Home Pay',
          `£${newResults.takeHomeAnnual.toFixed(2)}`,
          `£${(newResults.takeHomeAnnual / 12).toFixed(2)}`,
          `£${(newResults.takeHomeAnnual / 52).toFixed(2)}`,
          `£${(newResults.takeHomeAnnual / 260).toFixed(2)}`,
        ],
      ];
                            ...
                            <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                              <p className="text-sm text-yellow-800 dark:text-yellow-300">
                                ...
                                <strong>
                                  ...
                                  net annually, you need a gross salary of £
                                  {results.grossAnnual?.toLocaleString('en-GB', {
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-300">
                                  ...
                                  {bracket.bracketMax !== 'No limit'
                                    ? `£${bracket.bracketMax.toLocaleString()}`
                                    : bracket.bracketMax}
                                  {bracket.taxableAmount > 0
                                    ? ` | Taxed: £${bracket.taxableAmount.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                                    : ''}
                                </p>
                              </div>
                              ...
                              <div>
                                ...
                                <p className="text-sm text-gray-600 dark:text-gray-300">
                                  ...
                                  {bracket.niableAmount > 0
                                    ? ` | NI'able: £${bracket.niableAmount.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                                    : ''}
                                </p>
                              </div>
                        ...
                        {activeTab === 'netToGross' && (
                          <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                            <p className="text-sm text-green-800 dark:text-green-300">
                              ...
                              will give you your target net pay of £
                              {results.takeHomeAnnual?.toLocaleString('en-GB', {
                                maximumFractionDigits: 0,
                              })}
                              .
