...
export default function MortgageRepaymentCalculator() {
  ...
  const handleCalculate = useCallback(() => {
    ...
    setCsvData([["Month", "Principal", "Interest", "Balance"], ...schedule.map(r => [r.month, r['Principal Payment'].toFixed(2), r['Interest Payment'].toFixed(2), r['Remaining Balance'].toFixed(2)])]);
  ...
  return (
    <div className="bg-white">
        <div className="bg-gray-50 border-b"><div className="max-w-7xl mx-auto px-4 py-12"><h1 className="text-3xl font-bold text-center">Mortgage Repayment Schedule Calculator</h1></div></div>
        <div className="max-w-7xl mx-auto p-4 py-8">
            <div className="grid lg:grid-cols-5 gap-8">
                <div className="lg:col-span-2">
                    <Card><CardHeader><CardTitle>Mortgage Details</CardTitle></CardHeader>
                    <CardContent className="space-y-6">
                        <div><Label>Mortgage Amount (£{Number(loanAmount).toLocaleString()})</Label><Slider value={[Number(loanAmount)]} onValueChange={(v) => setLoanAmount(String(v[0]))} max={1000000} step={10000} /></div>
                        <div><Label>Interest Rate ({Number(interestRate).toFixed(2)}%)</Label><Slider value={[Number(interestRate)]} onValueChange={(v) => setInterestRate(String(v[0]))} max={20} step={0.1} /></div>
                        <div><Label>Term ({loanTerm} years)</Label><Slider value={[Number(loanTerm)]} onValueChange={(v) => setLoanTerm(String(v[0]))} max={40} step={1} /></div>
                    </CardContent>
                    </Card>
                </div>
                <div className="lg:col-span-3">
                    {results ? (
                        <div className="space-y-6 printable-area">
                            ...
                            <Card><CardContent className="p-4 grid md:grid-cols-2 gap-4 text-center">
                                <div><p className="text-sm">Monthly Payment</p><p className="text-2xl font-bold">£{results.monthlyPayment.toFixed(2)}</p></div>
                                <div><p className="text-sm">Total Interest</p><p className="text-2xl font-bold text-red-600">£{results.totalInterest.toLocaleString('en-GB', {maximumFractionDigits:0})}</p></div>
                            </CardContent></Card>
                            <Card><CardHeader><CardTitle>Loan Balance Over Time</CardTitle></CardHeader>
                                <CardContent><ResponsiveContainer width="100%" height={300}>
                                    <AreaChart data={results.schedule}><CartesianGrid /><XAxis dataKey="month" /><YAxis /><Tooltip /><Area type="monotone" dataKey="Remaining Balance" stroke="#3b82f6" fill="#3b82f6" /></AreaChart>
                        ...
                        <Card className="flex items-center justify-center h-full min-h-[300px]">
                            <CardContent className="text-center p-4">
                                <p className="text-gray-500">Enter your mortgage details to see the repayment schedule.</p>
