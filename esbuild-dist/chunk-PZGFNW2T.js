...
// src/components/data/calculatorConfig.jsx
var calculatorCategories = [
  {
      ...
      },
      {
        ...
        calculators: [
          ...
          // New calculator added here
          {
            ...
            icon: Target,
            status: "pending",
            description: "Calculate the minimum savings needed to stop contributing and still hit your retirement goal."
          }
        ]
      }
    ]
  ...
  (cat?.subCategories || []).forEach((sub) => {
    ...
    (sub?.calculators || []).forEach((calc) => {
      ...
      const uniq = Array.from(
        new Set(
          tags.map((t) => typeof t === "string" ? t : "").filter(Boolean).map((t) => toKebab(t)).filter(Boolean)
        )
      );
      while (uniq.length < 2) {
...
var allCalculators = calculatorCategories.flatMap(
  (category) => category.subCategories.flatMap((subCategory) => subCategory.calculators)
);
var getCalculatorStats = () => {
...
var getAllCalculators = () => {
  return calculatorCategories;
};
var searchCalculators = (query) => {
  const lowerQuery = query.toLowerCase();
  return allCalculators.filter(
    (calc) => calc.name.toLowerCase().includes(lowerQuery) || calc.description.toLowerCase().includes(lowerQuery)
  );
};
...
// src/utils/getRelatedCalculators.js
var OVERRIDES = {
  // Example:
  // '/some-calculator': [
...
function getRelatedCalculators(currentUrl, opts = {}) {
  const { max = 3 } = opts;
  ...
  const scored = all.map((c) => ({ c, s: score(c) })).filter((x) => x.s > 0).sort((a, b) => b.s - a.s).slice(0, max).map((x) => x.c);
